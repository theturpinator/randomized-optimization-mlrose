...
#print(np.unique(y_train, return_index=False))
#print(X_train)



# SOURCE: https://mlrose.readthedocs.io/en/stable/source/tutorial3.html#neural-networks
...
y_train_hot = one_hot.fit_transform(np.asarray(y_train).reshape(-1, 1)).todense()
y_test_hot = one_hot.transform(np.asarray(y_test).reshape(-1, 1)).todense()

#print(y_train_hot)


# perform cross validation
cross_val_scores = cross_val_score(m.model, X_train_scaled, y_train_hot, cv=cv_folds)
m.compile_stats(X_train_scaled, y_train_hot, cv_folds)



# fit model
#train_st = time.time()
m.model.fit(X_train_scaled, y_train_hot)
plt.plot(m.model.fitness_curve)
plt.title("Fitness Curve - MIMIC")
plt.savefig('nn_fitness_mimic.jpg')
plt.show()
